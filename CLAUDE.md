...
src/
  ...
  codex-message-manager.js — CodexMessageManager (Codex JSON-RPC/rollout → normalized messages; 2026-09-06 per-message meta at each response's token_count: requestId = the ledger's own `cx:<thread>:<cumulative>` key (never a vendor id — kind-marked), msgId = the 0.153 token_usage_record response id, fresh-input/reasoning/effort/model — the popup + billing row now resolve for codex; thread id = the RENDERED conversation, reader-pinned > wrapper_meta > FIRST session_meta — a sub-agent rollout carries the PARENT's at line 1, "last wins" mis-billed 11 local rollouts; identical re-emitted token_counts dedup like the walker; corpus smoke vs the walker over real local rollouts). ⇒ kb-file-structure.md
