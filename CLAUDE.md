...
src/
  ...
  collab-row.js — PURE(零依赖, CJS进bundle) codex 多agent collab 行的标签/HTML构造器: esc/t/图标全部注入 ⇒ XSS规则可被单测证明(marker escaper); 一行的纯文本与HTML同源不漂移; 行内永不携带加密载荷 ⇒ kb-file-structure.md
