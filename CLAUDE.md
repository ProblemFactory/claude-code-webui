...
src/
  ...
  search-card.js — PURE web-search card helper shared server+browser (2.369.43): renderSearchOutput (codex web_search_end → "title — url / snippet" blocks, page-action heads, 20/4KB caps; the normalizer's ONLY results renderer — the wrapper relays codex's typed event, never renders), searchQueryOf (the query/url a search card's TITLE chip shows, all harnesses), searchActionKey (0.14x rollout twin pairing). ⇒ kb-file-structure.md
