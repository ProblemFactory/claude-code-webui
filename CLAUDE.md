...
```
src/plugin-manifest.js — PURE plugin manifest validator (Plugin Ph2, 2.369.24): id=<publisher>.<name>=folder, semver, engines.vibespace min, client tier, server flag, contributes windows/agentTools/routes, declared capabilities; 2.369.43-.44 hardening = cleanText() on every free-text field (it reaches generated code), fs paths COLLAPSED **and their `*` wildcard MODELLED as the string prefix node matches** before the forbidden-root test (`/*` / `<parent>/*` used to pass), agentTools ⇒ needsConsent
