 ...
 *
 *  WHY CHUNKED and not one big glob: `/proc/[0-9]*` /fd expands to one operand
 *  per process, and the fd-level glob the old boot-restore probe used ALREADY
 *  overflows ARG_MAX on this machine (execve fails, stderr is discarded, the
 *  result is a silent empty list). Chunks can never reach that limit.
 *
...
vs_is_cli() {
  ...
  case "$vs_c_e" in */"$2"/versions/*) return 0;; esac
  return 1
}`;
}
