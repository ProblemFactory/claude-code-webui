...
const SUITES = [
  ...
  'test-init-frame', // §2.6 the claude init frame widened + commands_changed: the SCHEMA PIN (every consumed field re-grepped out of the installed 2.1.257 binary; evidence-SKIP without it), normalizer ops + REPLACE semantics + old-CLI degradation, the pure client rules (terminal-command filter / health strip / frame-declared memory dirs), the chatStatus attach twin, and the card in headless chrome (health strip visible while collapsed, XSS, 375×667)
  ...
  'test-stdout-registry', // S5 stdout consumer registry: descriptor caps.streamProtocol → ONE consumer (src/server/stdout/); unknown protocol = loud console.error + telemetry + RAW passthrough (never stream-json); each consumer on a fake pty feeds representative records to its REAL normalizer + id adoption / streaming flag / _stdin_ack / todos / engine calls; wiring pins. ALSO SPENDS ONE CHEAP TURN (round 4): the wire-reachability leg runs scripts/probe-claude-stdout.mjs — the installed CLI, the wrapper's exact flags, a read-only tool, --model haiku, VIBESPACE_* stripped so it cannot touch production state — because three suites once stayed green on a record that never arrives. No CLI / no tool ran / timeout ⇒ loud SKIP, never a failure
  ...
  'test-opencode-s9', // S9 REMAINDER (B-eac2): roll-back/restore, the ask card end to end, the serve terminal bridge, the live event lane that replaced the 10s list poll, honest 'external' liveness — mock serve + a real-binary section that SKIPS with evidence
  'test-opencode-remote', // S9 remainder (e): the OpenCode op TABLE is one definition — local facts, the `opencode-serve` agentd op and the shipped ssh script must not drift (parity)
  ...
  'test-permission-rules', // the READ-ONLY "where does this rule come from" view + the human-triggered local oracles (owner rulings 10 & 6): the PURE record/renderer over per-harness fixtures (XSS proved with a marker escaper, honest empty states), the server reader against real settings files + a fake live session (caps routing, the wrapper-advert skew gate, typed refusals), a REAL codex oracle run, and headless chrome at 375×667 through BOTH real doors (Manage Agents modal + Session Properties section)
