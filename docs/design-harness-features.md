...
16.（round 5 r2 自纠）「每一个状态栏下拉都在 `showDropdown` 里被夹一次」是**假的**：design / goal / set-a-goal 三处在它返回之后才写 min/max width，夹紧看不到那个宽度；round 5 的 kb 文案、报告与套件断言名都按两个样本（effort/permission）外推。现已改为「意图即参数」+ 源码级唯一写者钉 + 十面板普查（含裁切）。
