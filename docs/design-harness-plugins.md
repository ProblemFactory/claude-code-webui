...
| S9 ✅ (store facts: serve discover/reader/fork; 剩: revert/question/pty/SSE) | OpenCode serve-mode STORE: src/opencode-serve.js（127.0.0.1 client 1.5s 预算 + locator/keeper: data/opencode-serve.json 复用或首次 discovery 时懒启动 `opencode serve --port <free> --hostname 127.0.0.1`、backoff、5 次崩溃 parked、退出即停 + 'acp-events' 合成：text→agent_message_chunk/user、tool→tool_call+tool_call_update、reasoning→agent_thought_chunk）+ opencode.js store（discover 10s 缓存/10s 负缓存/挂起不阻塞轮询、createReader=serve 回填的 AcpSessionMessages 只读、forkSession=POST /session/{sessionID}/fork、forkChain=[]、locate 仍 null）；ws-create 先 mint fork id 再 resume（失败响亮拒绝，绝不退化成双写 resume）；capsOf('opencode').fork 只按 serve OpenAPI 证据翻（setVerifiedCap）+ 'harness-caps-updated' 广播；**未接**（1.18.29 OpenAPI 已见）：POST /session/{sessionID}/revert、GET /question + POST /question/{requestID}/reply、/pty*、SSE /event；'external'（别的进程在驱动同一 sqlite）不可探测 | test-opencode-serve（mock serve：client / 映射经真 normalizer / discover 缓存·负缓存·挂起 ≤2s / keeper 复用·spawn·崩溃 park·stop / serve reader / caps 判定 + wiring pins）+ 手动 scripts/dev/opencode-serve-live.mjs |
