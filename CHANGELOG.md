...
## 2.369.48
- **Image media cards (owner: "view image 能不能也多媒体化：可以展开直接看到图像内容，点开可以放大"; worktree agent, three verify rounds in headless chrome).** Every image a tool looked at is now an expandable, zoomable thumbnail: one DOM-free helper `imageMediaHtml` renders an open `<details class="chat-media">` with an SVG icon, basename and type/size chip and a lazy `/api/file/raw?path=…[&host=…]` thumbnail (remote sessions carry the host), and the existing click → zoom overlay applies. Wired for claude Read of an image (evidence-based: the LIFTED image blocks decide, never the extension — a `Read logo.svg` whose result is numbered text keeps its code-block card with a thumbnail appended below; 12/12 real .svg reads in the fleet are text), codex `view_image` (the wrapper's live stub, the ≤0.130 `view_image_tool_call` event and the 0.153.4 `item_completed {ImageView, path:'file://…'}` carrier — 48 in the corpus, ZERO cards before — all converge on ONE card per id, file:// decoded once; a live call whose output never came is completed by the rollout record), generic tools naming an image file, and ACP/claude inline image blocks. The 2.369.35 thumbnail path had never worked (the Read branch returned before the splice and the URL carried two stray quotes). Folding: the image card itself stays visible inside a mixed run — the Bash cards around it fold, the image member never gets `chat-run-collapsed` (it still carries the rail, first/last, sticky mark and the "1 image reads" count); a run made only of images builds no header at all; tiff/heic with lifted blocks show the chip without an unrenderable `<img>`; a broken thumbnail becomes "Image not available on this machine" via a capture-phase error delegate (no inline JS). Gate: scripts/test-image-cards.mjs (68), test-fold-ux mixed-run browser fixture with a real PNG served through /api/file/raw (70), test-codex-history view_image triple (140).

## 2.369.47
- **Opus 4-lens adversarial review, three more areas fixed (worktree agents, two verify rounds each, every fix negative-controlled on real data).** *Codex discovery / zst / remote fetch*: a compressed rollout whose text compresses better than ~4× got an EMPTY head (7 of 40 real rollouts) and VANISHED from the sidebar, with the empty meta cached — the head is now read by bisecting compressed prefixes under a fixed output cap, and an unreadable head throws instead of caching '' (`_noteExtractFailure` logs it once per file per 10 min); the per-directory listing cache records settledness at FILL time (a same-mtime-tick sibling rollout is no longer invisible forever); .jsonl/.jsonl.zst twins de-duplicate to the PLAIN twin with merged facts (the newest-first producer order had let the .zst win and drop the name/cwd); the remote transcript cache records the resolved remote path + compressed flag, never delta-appends onto a compressed file, heals slots the pre-fix code corrupted (pre-provenance meta ⇒ one whole refetch; the file's own bytes testify: zstd magic under a plain remote, JSONL head byte, NUL/zstd frame in the tail) and `find … | sort | head -1` makes the pick deterministic. *ACP wrapper*: a Stop landing during the context fetch no longer dispatches the prompt afterwards (cancelled re-checked after every await); Stop always clears the queued prompts ("queue-cleared" notice) and a dropped bookkeeping nudge no longer disables bookkeeping for the session; a Deny with no reject-kind option answers `cancelled` — never allow_always (the tool used to execute); an agent that exits 0 before answering initialize is reported instead of a blank window. *Plugin security*: agent-tool descriptions/args are embedded via JSON, control characters and line terminators are stripped (a CR injected code into the generated shim that runs outside the sandbox with the session token), agentTools now require consent; fs capability paths are normalized (`//`, `/./`, `~//` no longer bypass the forbidden-root guard) and node's trailing `*` wildcard is modelled (`/*` and wildcards over a forbidden root are refused); reinstalling a DIFFERENT package under a trusted id resets enabled+trust and requires re-consent (keyed on content hash + recorded source, even when the folder was absent); proxied plugin responses carry nosniff + a sandbox CSP; write-implies-read is stated in the consent summary; the install staging walk is async with a bound; a per-child stopping flag; uploaded temp files always cleaned. Seven small residuals parked as B-7638. Gate: test-codex-zst 44→86, test-acp-harness 72→91, test-plugin-trust 71→72, test-plugin-loader 47→48.

## 2.369.46
- Release gate: the headless-chrome suites (test-client-boot, test-run-collapse-fold, test-chat-e2e) hard-coded their server/CDP ports and dirs, so two gates on one machine collided and the loser waited out its budget — every "load" red on 2026-09-06 was this. They now take free ports and per-pid dirs, client-boot cleans up on SIGTERM/SIGINT (72 leaked worktrees found), and ci.mjs gives browser suites a 600 s budget (300 s elsewhere).
- **Review batch (Opus 4-lens adversarial review, ledger + chat-view area; worktree agent, verified).** Usage estimator memo: the interval-cost memo key now carries a price-table token (a pricing/discount edit no longer serves pre-edit dollars for every historical interval; no token ⇒ no memo), eviction changed from FIFO to a random victim (a cyclic learn pass 30 % past the 20000-entry cap kept 53 % hits instead of collapsing to 0 % — the cap is a fleet total reached in ~3 months at today's rate, not per identity), and the memo moved from a module-global Map to a per-ledger WeakMap (two ledgers can no longer read each other's costs). Chat view: the 2.369.36 post-attach short-view rescue (`rendered < 30`) was unreachable on the normal attach path (a tail(50) attach always renders ~50 cards); replaced by a rescue that needs two corroborating geometry readings across the settle window, aborts on any structural change, excludes teleport mode and carries a harm bound (`rendered + 50 ≤ 150`) so it can never cost the live tail; dispose() clears both timers. Refuted-and-recorded: packing memo values into a Float64Array (measured 565 vs 533 B/entry — no gain). Gate: test-usage-ledger-perf 11→21, test-chat-trim-guard 15→28.

## 2.369.45
- **Run folds: honest labels + an expanded run that stays legible past one screen (owner report 2026-09-06; worktree agent, two verify rounds in headless chrome).** ToolSearch was hard-mapped to the MCP kind, so a run of 9 Bash + 1 ToolSearch read "1 MCP" — a new PURE `src/lib/chat-run-summary.js` classifies it as a tool lookup ("N tool lookups"; it still folds under the MCP toggle) and the MCP count + "(server)" suffix come only from real mcp__server__tool calls; `mcpParts` lives there once. Expanded runs: every member carries a continuous accent rail (`.chat-run-member` + first/last, both role-indicator modes), a floating run bar pinned to the top of the chat viewport appears while the run's header is scrolled out (label + jump-to-top + collapse; collapsing lands the header exactly where the bar was), and a bottom "Collapse · …" line closes the run from its end. The message list stays flat (header/footer/bar are never `.chat-msg`; trims, minimap, search and the seek anchors skip them); the bar is computed inside the existing rAF scroll pass with one geometry read and never pages. Review round: expanding a non-last run while pinned re-collapsed itself 180 ms later (the footer insert woke the mutation observer whose pass auto-refolds non-last runs) — the footer mutation now drains its own observer records and a deliberately opened run is exempt from that pass (the mark rides element swaps like `_runExpanded`); the history-load pill drops below the bar. Gate: scripts/test-fold-ux.mjs (61 incl. a two-run pinned browser fixture with a negative control), test-owner-batch pin moved to the PURE module.

## 2.369.44
- **Codex web-search cards (owner screenshot: every card read `{"query": "", "action": null}` / "(empty)") + the search query in every search card's title; three worktree rounds, each adversarially verified against the real rollout corpus keyed by `cli_version`.** Root causes: the wrapper recorded the v2 `webSearch` item at item/started (an EMPTY stub) and never wrote the completed query/action/results back; and the installed codex 0.153.4 persists a search ONLY as `event_msg item_completed {item:{type:'Extension', kind:'web.search', id:'exec-…', query, action, results}}` — a carrier that sat in the normalizer's skip list (three real rollouts with 20/43/30 searches rendered ZERO cards; `web_search_end` exists only in ≤0.149.1 rollouts). Now: the wrapper relays codex's typed end event at item/completed (same exec-… id as the rollout item, so live + rebuilt land on ONE card); new PURE `src/search-card.js` is the single renderer ("title — url / snippet" blocks, `opened <url>` / `found '<pattern>' in <url>` heads, 20-result/4 KB caps, "no results" only for a literal `[]`, `web search (no details recorded)` for the one information-free 0.120 orphan shape); the 0.12x/0.130 end + id-less call twins pair positionally (59/200/106 cards on the real files, no empty twin); `_processItemCompleted` is an explicit allowlist router on `item.type` (typed twins of response_items skipped and named in a census, Extension web.search / image_gen and ImageView routed, SubAgentActivity terminal records — which twin nothing — now render the Sub-agent card, unknown types fire `codex-unknown-record:item_completed:<type>` once). Client: search cards (claude WebSearch/WebFetch, codex web_search/web_fetch, ACP) carry an escaped `.chat-tool-query` chip in the header (90 chars, full text on hover); codex raw names display as "Web search" / "Fetch page". Gate: scripts/test-search-card-title.mjs (47), test-codex-history (fixtures cut verbatim from 0.120/0.125/0.130/0.149.1/0.153.4 rollouts + whole-file pattern replays in scripts/fixtures/codex-web-search-patterns.json), test-codex-p2-wrapper stub extended to the real item/started+completed pair.

## 2.369.43
- **Codex per-message metadata + billing account (owner: "codex会话是不是依然看不到每条消息的详细信息、计费账号、使用模型"; three worktree rounds, each adversarially verified on the real rollout corpus).** The codex normalizer threaded no `meta`, so the message popup showed only role/time/uuid and the billing row fell to the session-level fallback. Now every response's `token_count` stamps the assistant/tool cards created since the previous one with claude-shaped meta: model, effort, usage (fresh input = input − cached, cache read, codex cache write, output, **reasoning tokens**), `requestId` = the ledger's own `cx:<thread>:<cumulative total>` key (derived identically, so already-scanned ledgers join without a rescan) and `msgId` = the 0.153 `token_usage_record` response id; live streams emit the same edit op claude uses. The ledger (usage-walker + the ssh single-file scanner, parity-pinned) bakes `mid` + `effort` with the rid unchanged; `/api/usage-stats/rid-info` returns be/model/effort and the popup gains Reasoning tokens / Effort / Ledger request key / Response ID rows; the popup's session-level billing fallback was dead for every backend (it read fields sessionAuth never produced) and now names the identity (pooled → target, ChatGPT login, subscription, api key). Corpus facts baked into the fixtures: a sub-agent rollout carries its OWN session_meta first and the parent's second (11 of 79 local rollouts — "last wins" keyed 2320 messages to the parent, 64 colliding with the parent's ledger events), codex emits duplicate `token_count` records (4.6 %), and merged fork reads prepend the parent's records — so the ledger thread id is now a fact about the RECORD's FILE (records are tagged with their rollout at merge time; the manager's thread id is only the default for provenance-less gap/tail reads), duplicates are deduped exactly like the walker, and `wrapper_meta.threadId` re-points a pinned default on live thread/fork. Also fixed on the way: `extractCodexThreadMeta` took forked_from_id from the LAST session_meta (the grandparent for sub-agent rollouts). Gate: test-codex-history 48→88 (real 0.149/0.153.4 fixtures incl. scripts/fixtures/codex-native-fork/, a 30-rollout corpus smoke asserting every minted key equals the walker's rid set), test-usage-walk-parity 21→32, test-codex-p2-wrapper 30→33.

## 2.369.42
- **Harness S9 — OpenCode serve-mode is the opencode harness's STORE (B-03f2 scoped; worktree agent, integrated; verified live against opencode 1.18.29).** The ACP harness has no transcript files (OpenCode keeps sessions in its own sqlite store), so stopped OpenCode conversations never reached the sidebar. New SHARED `src/opencode-serve.js`: a 127.0.0.1-only client over `opencode serve` (1.5 s timeouts, every failure a typed error; session listing = `/project` × `/session?scope=project&directory=<worktree>` — a bare `directory=` is an exact filter and listed nothing on the first live run; first-user-message names via the v2 message endpoint), a serve locator/keeper (reuse the recorded serve in data/opencode-serve.json when its health answers, else lazily spawn ONE `opencode serve` on a free port under agentEnv() on the first discovery, only when the CLI is installed; respawn with backoff, parked after 5 crashes, stopped on server exit; `VIBESPACE_OPENCODE_SERVE=0` or the smoke belt `VIBESPACE_SKIP_AGENT_HOOKS=1` disables autostart), and `messagesToAcpRecords` which rebuilds 'acp-events' so the UNCHANGED AcpMessageManager renders history (text/reasoning/tool/subtask/compaction/model-switch mapped; tool edits as diffs). The opencode descriptor's store: `discover` (10 s cache → 10 s negative cache keeping the last good list → refresh bounded by the 1.5 s budget; a hung serve resolves in <2 s and never throws; child sessions list as agentKind 'subagent'), `createReader` (live sessions keep the wrapper journal, stopped ones load from the serve), `forkSession` (POST fork; `capsOf('opencode').fork` flips true via the new PURE `setVerifiedCap` once the serve's OpenAPI confirms it, broadcast `harness-caps-updated`, the sidebar Fork row gates on META caps). ws-create mints the store fork id BEFORE spawning and resumes it (error code `fork-failed`, never degraded to a double-writer resume). Found on the way: `/api/session-messages` had no try/catch, so a loud reader refusal became a HUNG request — it now answers 404 (unknown conversation) / 502 (store unreachable) and the chat view's retry pill surfaces it. Integrator fix: the resume-already-live guard is now gated on the harness caps row (any chat harness with a stream protocol) instead of a claude|codex id list — a stale card could otherwise spawn a second `opencode acp` on one live session. Not wired (recorded with exact API names in the design doc): revert, question, pty, SSE push invalidation; 'external' liveness (a TUI driving the same session) is undetectable; local machine only. Gate: scripts/test-opencode-serve.mjs (98, mock serve incl. hung-serve bound + keeper crash-loop), test-writer-sweep §10 opencode pins; manual scripts/dev/opencode-serve-live.mjs.

## 2.369.41
- **Codex 0.153 remainder (B-21e4) + incident freeze reaches codex (B-8ebb); worktree agent, integrated; verified against the installed codex 0.153.4 bindings + a live `thread/read` probe.** ① Fork ordinal: `extractCodexThreadMeta` reads forked_from_id / forked_from_ordinal_exclusive / history_base / subagent_history_start_ordinal from the thread's OWN session_meta; `resolveCodexForkAncestry` prepends a Referenced fork's parent prefix cut at the boundary (the wrapper chain still merges whole; codex-native parents stay listed); the wrapper echoes forked_from_id and dedupes forked_from. ② Record tolerance: explicit skip sets (world_state, token_usage_record, inter_agent_communication_metadata, compacted, event/response_item twins); unknown types → telemetry `codex-unknown-record:<type>` once per process, never a fold-breaking card or a dropped turn; agent_message + sub_agent_activity render as complete 'agent'-fold tool cards (the old system line split every fold); old web_search_call / tool_search / local_shell / image_generation items routed (existing rollouts gain ~search cards that were invisible before); thread_settings_applied + turn_context.effort feed status().effort. ③ Effort `ultra`: offered in the three effort pickers only when the served model's supported_reasoning_levels report it, labelled "delegates to sub-agents (multi-agent), extra usage" (zh/ja). ④ Explicit model + effort on EVERY turn/start: the wrapper adopts an uncommanded effort from the thread/start|resume|fork reply so the app-server's per-thread defaults can never flip a resumed conversation; resume without an explicit effort carries the last turn's effort (the 2.369.32 model twin). ⑤ `thread/read` fallback (exists in 0.153.4; a live probe answered a real thread in 136 ms): new SHARED src/codex-thread-read.js = pure Thread→rollout-records mapper + ONE bounded `codex app-server` read (15 s, cached, negative-cached, local only, never a session) behind the codex descriptor's `store.warmTranscript`; ws-handler attach/view and transcript-service.view warm through the descriptor (claude parse-cache warm unchanged, the claude-only ternaries are gone); a stopped codex thread whose rollout file is missing still opens read-only. `thread/resume excludeTurns` + turns/items paging deliberately NOT adopted (our readers are whole-file/tail — recorded in the design doc). ⑥ Incident "freeze the scene": `captureLocal` iterates the harness registry's `store.locate` (codex rollout .jsonl/.jsonl.zst, local or remote-jsonl cache: size+mtime+sha256 + frozen tail, harness-tagged) and the remote probe is generated per (conversation × harness) from `store.remoteFind` — no backend ternary in incident.js. The thread-read configure() lives in src/server/cli-env.js (server.js back under the 2100-line ratchet). Gate: scripts/test-codex-0153.mjs (66, registered), test-incident +8, test-codex-history updated.

## 2.369.40
- **Wall → switch in the same tick (B-2c9b, plan A+B; worktree agent, integrated).** Three times on 2026-09-05 a walled turn's verdict read the POOL scope ("usable via <linked member>" off the linked member's healthy cache) while the limit banner had landed on the org the CLI was ACTUALLY on (a hot-switched CLI keeps its old token — 193 readings had been re-attributed away from the link); the pool only moved 1.5–3 min later when a reading arrived, and in-flight subagents died in the gap. (A) **A wall is ground truth for the account it lands on; readings only confirm.** Every wall signal (rate_limit_event, limit banner, the codex sites) now carries the org-verified cache key its mark landed on, and `demoteWalledAccount` runs FIRST in `onWalledTurn`: if that account is a member of the session's pool, the affected bucket is written exhausted NOW through the same write path readings use (`captureRateLimitEvent`, source `wall`: utilization 1, resetsAt = the signal's future reset, else the cached future reset, else the bounded 5h/24h guess), and the pool evaluation runs in the same `noteTurnEnd` AFTER the auto-resume arm, so a hot switch's fireNow continues the armed session immediately. Misattribution guard (the 2.368.34 class): demote only when the account saw ≥2 walled turns inside 120 s (any sessions, one entry per turn) OR it is the session's OTel-observed org (≤10 min); a single unverified wall holds — journal `[wall] … holding the demotion` + telemetry `wall-demote-held`; demotions log `[wall] demoted <name> <bucket> until <iso> (n walls / observed-org)` + `wall-demote`. (B) **Observed-org aware evaluation.** `sessionCurrentMember` (OTel truth ≤10 min → pool member, else the link) now drives `resolveUsageKey` for pooled sessions (live odometer, probe matching, every derived cache key), `quotaVerdictFor(scope, {model, session})` (the session's current member is judged first so `via` names what it is on; verdict carries on/linked/divergent), the per-session auto-switch pass (decides FROM the observed member; a same-target re-point is journal-only + creds-mtime bump) and both probe targets; divergence logs once per session per 10 min (`[pool] session <id> observed on <X> while linked to <Y>`, telemetry `pool-observed-divergence`). The hot-switch symlink still targets the chosen member. No product default changed. Known limit: a live CLI can hold its old token ≥25 min, so a re-pointed session may re-wall once — the second wall inside 120 s demotes (self-limiting). Gate: test-auto-resume §11 (real pool + real engine + real auto-resume, 125), test-rate-limit-capture ⑧ (42); design record docs/design-wall-machine.md addendum.

## 2.369.39
- **Harness S5 — stdout pipeline per descriptor** (docs/design-harness-plugins.md §2.4 S5; worktree agent, integrated). The three inline branches of `setupSessionPty` (claude stream-json / codex-events / acp-events, ~830 lines) moved VERBATIM into `src/server/stdout/<protocol>.js` behind a protocol registry (`src/server/stdout/index.js`): the harness descriptor NAMES the protocol (`caps.streamProtocol` stays the ONE source of truth, pinned identical by harness-contract), the registry resolves it to `create(deps) → { protocol, attach(session, id, pty, helpers) }`, and session-stdout.js (1169→352 lines) does nothing but look it up and attach. A declared protocol with NO registered consumer used to fall to raw passthrough silently — it now fails LOUDLY at session start (console.error naming backend/protocol/fix + telemetry `chat-protocol-no-consumer`, output still RAW, never mis-parsed as stream-json); the no-protocol text is unchanged. Verbatim proof: every moved body diffed identical modulo indentation against HEAD, and every free identifier each body uses is declared in its new header (the lost-binding class). Deliberate deviation from the design's "descriptor.stream.parse shared with the daemon": the consumers need the pool/quota engine, hosts.harvestUsage, the ledger, goal sync and the delivery ladder, so they stay ORCH-tier; session-brain's claudeSideEffects/sbSeenFirst wiring is untouched. Gate: scripts/test-stdout-registry.mjs (58: registry shape, descriptor↔consumer coverage, every consumer driven on a fake pty with REAL normalizers, unknown-protocol loudness) + harness-contract pins; attach-rebuild/session-schema/chat-frame-guard pins now scan src/server/stdout/.

## 2.369.38
- **Plugin Ph1 complete — commands + menus + keybindings registry, ws `default` loud case** (docs/design-harness-plugins.md §3.3 Ph1; worktree agent, integrated). New `src/lib/contributions.js` (DOM-free at import): `registerCommand`, `registerMenuItem` (menu/group/order/`when`, explicit separators, submenus, decorate), `menuItems(menu, ctx)` → showContextMenu-shaped items with VS Code ordering, `registerKeybinding` + ONE document-level dispatcher under an app-lifetime AbortController, `runCommand`; every register returns dispose() and honours a signal; malformed/duplicate registrations throw, unknown commands warn once + telemetry. Core dogfoods it with ZERO behaviour change, proven by identity tests against the verbatim legacy builders (session-card menu over 2048 states, window menu over 320, gear menu over 16 incl. the plugin-windows block): session-card.js `session.*` commands + `registerSessionCardMenu()`, taskbar.js `window.*` + `registerWindowMenu()`, new `src/lib/gear-menu.js` (`registerGearMenu`/`buildGearMenu`, shared with mobile-nav), command-mode.js and session-palette.js route every action through `runCommand` while keeping their modifier-lenient capture-phase key dispatch (a strict chord migration would change behaviour — documented). Plugin host API gains `api.registerCommand / registerMenuItem / registerKeybinding / runCommand`, ids namespaced `plugin:<id>:<slug>`, removed on deactivate (docs/plugins.md). Server: ws-handler's `switch(data.type)` has a `default:` — one log line per unknown type per 10 min, telemetry `ws-unknown-type`, reply `{type:'error', code:'unknown-type'}` deliberately without sessionId (every client error consumer is sessionId/reqId-gated, so it can never flip a window read-only). Gate: scripts/test-contributions.mjs (133). Loader-suite pin updated for the gear-menu move.
